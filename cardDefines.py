...
OffensiveCards = {
    ...
    "early assault": Card(12, "Early Assault", 30, 65, 5, "(Placeholder text here.)", "The Card's base Power doubles in the first 6 turns of the battle, and halves by Turn 18 onwards.", "offensive"),
    "sniper shot": Card(13, "Sniper Shot", 35, 60, 7, "Gottem! Wait... oh.", "Increased critical hit damage.", "offensive"),
    "chain breaker": Card(15, "Chain Breaker", 20, 100, 4, "```User 3: lol\nUser 4: lol\nMod: lmao no```\n", "Breaks the target's chain after the move, decreasing Defense equal to target's chain level. Power is multiplied by 50% for each chain level. Fails if the target does not have a chain.", "offensive"),
    "rupture": Card(17, "Rupture", 999, 30, 8, "Cracks open the ground below the target, potentially killing them.", "No additional effects.", "offensive")
}
...
DefensiveCards = {
    ...
    "chain": Card(14, "Chain", -1, 90, 3, "```User 1: lol\nUser 2: lol\nUser 3: lol```\n", "Increases user's Defense by 15%. For each successive use, elevates chain level by 1.", "defensive"),
    "revival": Card(16, "Revival", -1, 60, 3, "lol let's make chat alive again.", "Allows the user to revive and be healed by 50% HP should the user get KO'ed. Fails while user's **Revivable** status is active.", "defensive"),
    "protect": Card(18, "Protect", -1, 90, 4, "\"I protect the hooman\" - cat", "Greatly reduces an opponent's attack damage by 85%. If it's an OHKO attack, it only reduces the user's HP to 1. If it's a status-applying Card, it cancels it. Successive use halves the Card's accuracy. Fails if the user is poisoned.", "defensive")
}
...
MiscCards = {
    ...
    "restore": Card(1000, "Restore", -1, 100, 0, "Wait, that's illegal", "Restores the User's energy. Alias: `c.restore`", "miscellaneous")
}
